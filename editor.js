class Editor {
  ...
  selectText(text) {
    const content = this.editor.getText(); // Get all text from the editor
    const index = content.indexOf(text); // Find the first occurrence of the text

    if (index !== -1) {
      this.editor.setSelection(index, text.length, 'api'); // Set selection
